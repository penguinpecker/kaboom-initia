...
import { useVaultMaxBet, useWalletBalance } from "@/hooks/useContracts";
import { GAME_CONFIG } from "@/lib/chain";
import { formatEther } from "@/lib/compat";

export function BetControls() {
  const { state, setBet, setMineCount, startGame, cashOut, enableAutoSign } = useGame();
  ...
  const maxBet = maxBetWei ? Number(formatEther(maxBetWei as bigint)) : 999;
  const walletBalance = walletBalData ? Number(walletBalData.formatted) : 0;
    ...
    <div className="space-y-6">
      <section className="bg-surface-container-low p-6 stealth-card border border-outline-variant/10">
        ...
        <div className="space-y-4">
          <div>
            <label className="font-headline text-[10px] uppercase tracking-widest text-on-surface-variant mb-2 block">Bet Amount (INIT)</label>
            <div className="relative">
              <input
                type="number" step="0.01" min="0.001" value={state.bet}
            ...
            <div className="flex justify-between mt-1 text-[9px] font-headline text-on-surface-variant/40">
              <span>Balance: {walletBalance.toFixed(3)} INIT</span>
              <span>Max bet: {maxBet.toFixed(4)} INIT</span>
            </div>
          </div>
          <div>
        ...
        {/* Auto-sign nudge, only when the user hasn't enabled it yet */}
        {isConnected && !state.autoSignEnabled && !isPlaying && (
          <button
            onClick={enableAutoSign}
            className="mt-6 w-full flex items-center justify-center gap-2 py-3 bg-tertiary/5 border border-tertiary/20 text-tertiary font-headline text-[10px] font-bold uppercase tracking-widest hover:bg-tertiary/10 transition-all"
          >
            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>bolt</span>
            Enable Session Signing — instant tile clicks
          </button>
        )}
        ...
        {!isPlaying && state.status !== "cashing" ? (
          <button
            ...
            className="w-full mt-4 py-5 bg-gradient-to-r from-primary to-primary-container text-on-primary font-headline font-black text-lg tracking-[0.2em] glow-primary hover:brightness-125 transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
          >
            {isStarting ? (
          ...
          <button
            ...
            className="w-full mt-4 py-5 border-2 border-emerald text-emerald font-headline font-black text-lg tracking-[0.15em] hover:bg-emerald/10 transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
          >
            {isCashing ? (
              ...
              <>
                ...
                EXIT &amp; WITHDRAW — {(state.bet * state.multiplier).toFixed(4)} INIT
              </>
            )}
          </button>
        )}
      ...
      {state.commitment && state.status === "playing" && (
        <section className="bg-surface-container-low p-4 border border-outline-variant/10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-headline text-[10px] font-bold tracking-widest text-emerald uppercase">keccak256 Commitment</span>
          </div>
