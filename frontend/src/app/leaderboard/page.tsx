    ...
    <div className="px-6 lg:px-8 pb-16 min-h-screen">
      <div className="flex justify-between items-end mb-8">
        <div>
          <p className="font-headline text-[10px] tracking-[.12em] text-on-surface-variant flex items-center gap-1 mb-1">
            <span className="status-dot" />SYSTEM_NODE: {address ? address.slice(0, 10).toUpperCase() : "NOT_CONNECTED"}
          </p>
        ...
        <div className="hidden lg:flex gap-4">
          <div className="bg-surface-container-high p-4 border-l-2 border-primary">
            <div className="font-headline text-[10px] tracking-widest text-on-surface-variant uppercase mb-1">Total PnL (session)</div>
            <div className={`font-headline text-2xl font-bold ${localStats && localStats.totalPnl >= 0 ? "text-primary" : "text-error"}`}>
              {localStats ? (localStats.totalPnl >= 0 ? "+" : "") + localStats.totalPnl.toFixed(3) : "+0.000"} INIT
            </div>
          </div>
          <div className="bg-surface-container-high p-4 border-l-2 border-tertiary">
            ...
            <div className="font-headline text-2xl font-bold text-tertiary">{activePlayers || (gameCount ? gameCount.toString() : "0")}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-8">
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg overflow-hidden">
              ...
              {["Daily", "Weekly", "All-Time"].map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-3 py-1 font-headline text-[10px] font-bold tracking-widest transition-colors ${
                    t === tab ? "bg-primary/10 text-primary border border-primary/15" : "text-on-surface-variant/40 hover:text-on-surface"
                  }`}>{t}</button>
              ))}
            </div>
          </div>
          ...
          {onChain.length > 0 ? onChain.slice(0, 10).map((entry, i) => {
            const isMe = address && entry.player.toLowerCase() === address.toLowerCase();
            return (
              <div key={entry.player} className={`grid grid-cols-5 px-6 py-4 items-center border-b border-outline-variant/[0.04] ${isMe ? "bg-primary/5" : "hover:bg-surface-container-highest transition-colors"}`}>
                <RankBadge rank={i + 1} />
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-headline text-xs font-bold text-on-surface">{isMe ? "YOU" : entry.player.slice(0, 8) + "…"}</div>
                    {isMe && <div className="font-headline text-[9px] text-on-surface-variant/40">{address}</div>}
                  </div>
                </div>
                <span className="font-headline text-sm font-bold text-primary">{Number(formatEther(entry.biggestWin)).toFixed(3)} INIT</span>
                <span className="font-headline text-sm font-bold text-secondary">{(Number(entry.biggestMultiplier) / 10_000).toFixed(2)}×</span>
        ...
        <div className="space-y-6">
              ...
              {recentWins.length > 0 ? recentWins.map((g) => (
                <div key={`${g.gameId}-${g.timestamp}`} className="px-5 py-3">
                  ...
                  <div className="flex justify-between items-center mt-1">
                    <span className="font-headline text-sm font-bold text-primary">+{g.payout.toFixed(3)} INIT</span>
          ...
          <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-5">
            ...
            {localStats ? (
              <div className="space-y-3">
                ...
                <StatRow label="Biggest Win" value={localStats.biggestWin.toFixed(3) + " INIT"} color="text-tertiary" />
                <StatRow label="Best Mult" value={localStats.biggestMult.toFixed(2) + "×"} color="text-amber" />
                <StatRow label="Total PnL" value={(localStats.totalPnl >= 0 ? "+" : "") + localStats.totalPnl.toFixed(3) + " INIT"} color={localStats.totalPnl >= 0 ? "text-primary" : "text-error"} />
              </div>
          ...
          <div className="bg-gradient-to-br from-secondary-container/20 to-surface-container-high p-6 border border-secondary/10 rounded-lg">
            <h3 className="font-headline text-xl font-black italic text-on-surface mb-2">INTERWOVEN REWARDS</h3>
...
function RankBadge({ rank }: { rank: number }) {
  const cls = rank === 1 ? "bg-amber/15 text-amber border-amber/30"
    : rank === 2 ? "bg-on-surface-variant/10 text-on-surface-variant border-on-surface-variant/20"
    : rank === 3 ? "bg-tertiary/10 text-tertiary border-tertiary/20"
