        ...
        <div className="hidden lg:flex gap-4">
          <div className="bg-surface-container-high p-4 stealth-card border-l-2 border-primary">
            ...
            <div className={`font-headline text-2xl font-bold ${state.sessionPnl >= 0 ? "text-primary" : "text-error"}`}>
              {state.sessionPnl >= 0 ? "+" : ""}{state.sessionPnl.toFixed(3)} INIT
            </div>
          </div>
          ...
          <div className="bg-surface-container-high p-4 stealth-card border-l-2 border-tertiary">
            ...
            <div className="font-headline text-2xl font-bold text-tertiary">{vaultHealth !== undefined ? vaultHealth.toString() : "—"}%</div>
          </div>
        </div>
      </div>
          ...
          <section className="bg-surface-container-low p-6 stealth-card border border-outline-variant/10">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/10">
                ...
                <span className="font-headline text-[10px] text-primary">{vaultBal ? Number(formatEther(vaultBal as bigint)).toFixed(2) : "—"} INIT</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/10">
                <span className="font-headline text-[10px] text-slate-400">Total Games</span>
                <span className="font-headline text-[10px] text-secondary">{gameCount !== undefined ? gameCount.toString() : "—"}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/10">
                <span className="font-headline text-[10px] text-slate-400">Vault Health</span>
                <span className="font-headline text-[10px] text-emerald">{vaultHealth !== undefined ? vaultHealth.toString() : "—"}%</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="font-headline text-[10px] text-slate-400">Chain</span>
                <span className="font-headline text-[10px] text-on-surface-variant">Initia MiniEVM</span>
              </div>
            </div>
          </section>
        </div>
