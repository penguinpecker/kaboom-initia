...
export default function LogsPage() {
  ...
  const filtered = useMemo(() => filter === "wins" ? history.filter(l => l.won) : history, [history, filter]);
  ...
  const maxBet = maxBetWei ? Number(formatEther(maxBetWei as bigint)).toFixed(4) : "0.0000";
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="font-body text-sm text-on-surface-variant max-w-lg">
            Comprehensive archive of all tactical engagements. Cached locally with on-chain tx verification via InitiaScan.
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setFilter("all"); setPage(1); }}
            className={`px-4 py-2 font-headline text-[10px] font-bold tracking-widest transition-colors ${
              filter === "all" ? "bg-primary/10 text-primary border border-primary/15" : "text-on-surface-variant/40 hover:text-on-surface"
            }`}>ALL GAMES</button>
          <button onClick={() => { setFilter("wins"); setPage(1); }}
            className={`px-4 py-2 font-headline text-[10px] font-bold tracking-widest transition-colors ${
              filter === "wins" ? "bg-primary/10 text-primary border border-primary/15" : "text-on-surface-variant/40 hover:text-on-surface"
            }`}>WINS ONLY</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-8 mb-8">
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-6">
          ...
          <div className="grid grid-cols-3 gap-8">
            <div>
              ...
              <span className={`font-headline text-3xl font-bold ${stats.pnl >= 0 ? "text-primary" : "text-error"}`}>
                {stats.pnl >= 0 ? "+" : ""}{stats.pnl.toFixed(3)} INIT
              </span>
        ...
        <div className="bg-surface-container-low border border-outline-variant/10 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center"><span className="font-headline text-[10px] text-on-surface-variant uppercase tracking-widest">Min Bet</span><span className="font-headline text-sm font-bold text-on-surface">0.001 INIT</span></div>
            <div className="w-full h-1 bg-surface-container-highest rounded"><div className="h-full bg-primary rounded w-[10%]" /></div>
            <div className="flex justify-between items-center"><span className="font-headline text-[10px] text-on-surface-variant uppercase tracking-widest">Max Bet</span><span className="font-headline text-sm font-bold text-on-surface">{maxBet} INIT</span></div>
          </div>
        ...
        {paged.length === 0 ? (
          ...
          paged.map((g) => (
            <div key={`${g.gameId}-${g.timestamp}`} className="grid grid-cols-7 px-6 py-4 items-center border-b border-outline-variant/[0.04] hover:bg-surface-container-highest transition-colors">
              ...
              <span className="font-headline text-sm text-on-surface">{g.bet.toFixed(3)} INIT</span>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`font-headline text-sm font-bold ${g.won ? "text-primary" : "text-error"}`}>{g.won ? "+" : "-"}{(g.won ? g.payout : g.bet).toFixed(3)} INIT</span>
              </div>
              ...
              <div className="text-right">
                {g.txHash ? (
                  <a href={`${EXPLORER}/tx/${g.txHash}`} target="_blank" rel="noreferrer"
                     ...
                     style={{ fontSize: 18 }}>visibility</a>
          ...
          <div className="flex gap-1">
            ...
            {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => (
              <button key={i} onClick={() => setPage(i + 1)}
                className={`w-8 h-8 flex items-center justify-center font-headline text-xs font-bold transition-colors ${
                  page === i + 1 ? "bg-primary/10 text-primary border border-primary/15" : "text-on-surface-variant/30 hover:text-on-surface"
                }`}>{i + 1}</button>
            ))}
