...
import {
  useVaultBalance, useVaultMaxBet, useVaultMaxPayout, useVaultHealth,
  useRiskLevel, useDepositToVault, useGameCounter,
} from "@/hooks/useContracts";
import { useToast } from "@/hooks/useToast";
import { CONTRACTS, EXPLORER, INITIA_CHAIN_PRETTY_NAME } from "@/lib/chain";
...
export default function VaultPage() {
  ...
  const handleDeposit = async () => {
    toast("Depositing " + fundAmt + " INIT...", "primary");
    await deposit(fundAmt);
    if (isSuccess) toast("Successfully deposited " + fundAmt + " INIT!", "emerald");
  };
  ...
  return (
    <div className="px-6 lg:px-8 pb-16 min-h-screen kinetic-grid">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StatCard label="Balance" value={fmt(balance as bigint | undefined) + " INIT"} color="border-primary" valColor="text-primary" />
        <StatCard label="Max Bet" value={fmt(maxBet as bigint | undefined) + " INIT"} color="border-secondary" valColor="text-secondary" />
        <StatCard label="Max Payout" value={fmt(maxPayout as bigint | undefined) + " INIT"} color="border-tertiary" valColor="text-tertiary" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
        <div className="bg-surface-container-low p-6 border border-outline-variant/10 stealth-card">
          ...
          <div className="space-y-2.5">
            ...
            <InfoRow label="Total Games" value={gameCount !== undefined ? gameCount.toString() : "—"} valueColor="text-secondary" />
            ...
            <InfoRow label="Chain" value={INITIA_CHAIN_PRETTY_NAME} valueColor="text-primary" />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-surface-container-low p-5 border border-outline-variant/10">
            ...
            <input
              type="number" step="0.1" value={fundAmt}
            ...
            >
              {isPending || isConfirming ? "CONFIRMING..." : "DEPOSIT INIT"}
            </button>
            {isSuccess && (
              <div className="mt-3 p-4 bg-emerald/10 border border-emerald/30 rounded-lg text-center">
                ...
                <span className="font-headline text-xs text-emerald/70">{fundAmt} INIT sent to vault</span>
              </div>
            )}
          </div>
